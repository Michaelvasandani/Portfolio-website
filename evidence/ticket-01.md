...
- Fixtures: all 51 stable catalogue IDs in the [versioned fixture catalogue](../fixtures/v1/catalog.json) passed their machine-readable contract assertions. Later tickets implement the source, generation, check, and recovery capabilities that execute those contracted outcomes. Run `pnpm test:fixtures -- --reporter=verbose` for the per-ID contract matrix.
